...
import { BarChart3, TrendingUp, Zap, Info, ArrowUpRight, Calendar, Brain } from 'lucide-react'
...
export default function UsageAnalytics() {
    ...
    const [usageData, setUsageData] = useState<{ series: any[], dates: string[] }>({ series: [], dates: [] })
    ...
    const fetchUsageData = async () => {
        setIsLoading(true)
        try {
            // Mock data for now since we don't have a trend API yet
            // In a real scenario, we'd call an endpoint for consumption history
            ...
            const series = [{
                ...
                data: Array.from({ length: Number(selectedPeriod) }, () => Math.floor(Math.random() * 500) + 100)
            }]

            setUsageData({ series, dates })
        } catch (error) {
    ...
    const hasData = usageData.series.length > 0 && !isLoading

    const totalUsage = useMemo(() => {
        if (!usageData.series[0]?.data) return 0
        return usageData.series[0].data.reduce((a: number, b: number) => a + b, 0)
    }, [usageData])
            ...
            <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-8 mb-8">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-black text-gray-400 mt-2 tracking-wider uppercase">Your Usage History</p>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-8 mr-4 lg:border-r border-gray-100 dark:border-gray-800 pr-8">
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] font-black text-gray-400 mb-1 flex items-center gap-1.5 uppercase">
                                <Zap className="w-3 h-3 text-amber-500" /> Total Usage
                            </span>
            ...
            <div className="relative z-10 mt-10 pt-8 border-t border-gray-50 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-8">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[11px] font-black text-gray-500 tracking-wide uppercase">Live Updates</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <Calendar className="w-4 h-4 text-gray-400" />
                        <span className="text-[11px] font-black text-gray-400 tracking-wide uppercase">Daily View</span>
                    </div>
                </div>

                <div className="flex items-center gap-4">
                    <span className="text-[10px] font-black text-gray-400 italic">Data updated 5m ago</span>
                </div>
            </div>
        </Card>
    )
